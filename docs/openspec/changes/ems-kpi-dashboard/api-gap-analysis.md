...
本文件详细梳理了监控看板前端原型页面与后端 API 接口的对应关系，识别出缺失接口，并设计了数据结构，以在前端 mock adapter / fixtures 中填补这些缺口。

## 0. 结论：现有接口不能完全覆盖四张页面

现有正式接口文档标题为《第三页：能源与碳排接口与表结构梳理》，实际只覆盖 `/api/kpi/energy/*` 相关能力，因此不能完整支撑四张原型页面的所有展示数据。

- **可直接覆盖**：能源与碳排页的大部分基础数据，包括设备下拉、单箱能耗、单箱成本、单箱碳排、传统电力 vs 绿电、设备百公里能耗的基础字段。
- **部分覆盖**：能源与碳排页中的能耗趋势、单车百公里能耗表。官方接口有基础单序列字段，但原型需要“调度前 / 调度后”双序列或双列对比字段。
- **未覆盖**：作业效率、关系网、调度分析、全局顶栏告警，以及能源页的分时电价模块。
- **首期开发方式**：不需要在本仓库开发生产后端。前端优先调用真实接口；真实接口没有覆盖的页面数据，首期用前端 mock adapter / fixtures 保证高保真页面展示，并在代码中保留与后续真实接口替换一致的数据契约。

因此，“只开发前端”是可行的，但前提是接受缺失接口先用前端 mock 数据支撑；若要求所有数据都来自真实服务，则仍需要后端补齐下表中的 MISSING/GAP 接口。
...
* **界面需求**：显示“调度前总电费”(RMB)、“调度后总电费”(RMB)、“节省金额”(RMB)、“平均电价”(RMB/km)、“避峰率”(%)。下方展示 6 个分时时段的电量与电费对比列表：`00:00-02:00`、`02:00-08:00`、`08:00-12:00`、`12:00-18:00`、`18:00-22:00`、`22:00-24:00`。
* **数据结构响应示例**：
```json
{
  "code": 0,
...
## 3. 前端 mock adapter 设计策略

为了在只做前端的首期范围内保证页面完整性，前端应构建可替换的数据访问层：
1. **真实接口优先**：`/api/kpi/energy/*` 优先通过 Vite proxy 调用 `http://10.105.64.36:20003/api`。
2. **缺口 fixture 补齐**：作业效率、关系网、调度分析、顶栏告警、分时电价和调度前后对比字段先由 `src/mocks` 或同等 fixture 模块提供。
3. **统一 API 形状**：前端服务层按 `openapi.yaml` 的契约输出页面模型，兼容真实后端 direct body 和 mock envelope `{ code, message, data }`。
4. **计算公式绑定**：
   - **能耗成本** = 能耗值 × 对应价格
     - 电价：按分时时段 fixture 或真实服务返回值计算
     - 柴油：`7.50 RMB/L`
   - **碳排放** = 能耗值 × 碳排系数
     - 电力系数：`0.527 kgCO2/kWh`
     - 柴油系数：`2.63 kgCO2/L`
5. **无缝替换**：前端统一基于请求工具 `apiClient.ts` 发起请求，后续后端补齐缺失接口时，只需切换对应 service 的数据源。
