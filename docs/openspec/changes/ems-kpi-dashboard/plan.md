...
本计划基于 4 张原型图、已上传的能源与碳排接口文档，以及 2026-05-22 复核结论更新。结论：总体方向同意，但必须按本版修正后再进入开发。前端必须以原型为高保真设计源；当前阶段先不以接口联调为核心，先完成视觉与交互规范确认。
...
- **详细规范**：开发前必须阅读并遵守 `frontend-fidelity-spec.md`，该文件优先级高于普通实现偏好。
...
在用户确认文档前，不进入前端代码开发。确认后按以下顺序执行：

1. 先搭建 `ems-frontend`，不创建生产后端。
2. 先用 fixtures 完整驱动四页，不等待接口。
3. 每完成一页，立即浏览器截图并与原型对比。
4. 所有核心交互走通后，才考虑接入真实接口。
